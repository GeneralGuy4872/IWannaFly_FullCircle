 ...
 * - ++ or -- move in z
 * - < > strafe
 * - f fire (requires a bow)
 * - F <cursor prompt> fire at something
 * - ⌤F fly
 * - ⌤L land
 * - J jump 2 spaces forward, in an arc
 * - j <number, strafe, spacebar, or backspace> jump 2 spaces, in an arc
 * - jj jump up (useful for climbing ladders)
 * - backspace retreats
 * - spacebar advances
 * - a slash
 * - A thrust
 * - ⌤A <same options for j> jumping slash attack (movement in an arc, targeting in a line)
 * - s raise shield
